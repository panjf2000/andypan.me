...
内核中 lag 的计算公式是 \\(lag_i = S - s_i\\)，其中 \\(S\\) 是理论上每一个进程应得的(物理) CPU 时间，\\(s_i\\) 是进程 \\(i\\) 实际已消耗的(物理) CPU 时间。通过前面的内容我们知道 \\(v_i = \frac{NICE\\_0\\_LOAD}{w_i} \times s_i\\) 和 \\(V = \frac{NICE\\_0\\_LOAD}{w_i} \times S\\)，其中 \\(w_i\\) 是进程的权重，\\(V\\) 是 \\(S\\) 对应的虚拟时间，\\(v_i\\) 是进程的虚拟时间。因此 lag 的计算公式可以进一步推演为 \\(lag_i = S - s_i = w_i \times (V - v_i)\\)。\\(NICE\\_0\\_LOAD\\) 是一个常数 1024，可以在公式中隐去。
