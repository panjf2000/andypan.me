...
这是另一个 EEVDF 核心的概念，***virtual deadline*** 表示虚拟截止时间，我们定义其为 \\(V(d)\\)，eligible time 为 \\(V(e)\\)，\\(V(d)\\) 必须满足以下约束条件：任务 \\(i\\) 发起调度请求时期望的 CPU 时间 \\(r\\) 等于 \\([V(e), V(d)]\\) 累加 CPU 时间，也即是 \\(S_i(e, d) = r\\)，其中 \\(r\\) 是任务发起调度请求时期望的 CPU 时间长度。通过对这个公式应用方程 (6) 可得：
